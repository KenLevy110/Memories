...
3. **CORS on the API:** Set Cloud Run env `WEB_ORIGIN` to a comma-separated list of exact browser origins that call the API (include `https://<your-site>.web.app`, `https://<your-site>.firebaseapp.com`, and any custom domain). The API echoes `Access-Control-Allow-Origin` only for allowlisted origins in production. The [Deploy API workflow](../.github/workflows/deploy-api.yml) reads this value from the GitHub Actions repository **variable** `WEB_ORIGIN` (not a secret) and bakes it into the Cloud Run revision; the deploy job fails fast if `WEB_ORIGIN` is unset, since browser fetches would otherwise be blocked and the web UI would only show `Network request failed.`
4. **Media bucket CORS:** Browser **PUT** uploads to **GCS signed URLs** require the bucket CORS config to list the same Hosting origins. The canonical config lives at [`infra/gcp/cors.json`](../infra/gcp/cors.json) and is applied automatically:  
   - **On bootstrap:** [`infra/gcp/bootstrap.sh`](../infra/gcp/bootstrap.sh) applies it after creating the bucket and grants the deploy SA bucket-scoped `roles/storage.admin` so future deploys can re-apply without re-running bootstrap.  
   - **On every API deploy:** [`deploy-api.yml`](../.github/workflows/deploy-api.yml) re-applies it after the Cloud Run revision goes live, so changes to `cors.json` (e.g. adding a custom domain) propagate without a manual `gcloud` step.  
   For a one-off manual apply (e.g. before the next deploy), run from a machine with `gcloud`:  
   `gcloud storage buckets update gs://<bucket> --cors-file=infra/gcp/cors.json`
5. **Custom claims (v0.5 manual path):** The API expects custom JWT claims `practice_id`, **`actor_user_id`**, `client_id` (or `client_ids`), and a **Guide** role for upload/finalize mutations (`GUIDE`, `GUIDE_PRIMARY`, or `GUIDE_SUPPORT`). Postgres stores `practice_id` and `actor_user_id` as **UUID** columns, so raw Firebase UIDs cannot be inserted as those values. The operator script derives stable UUIDs (UUID v5) from each Firebase UID and writes them as custom claims:
   ...
   - **Firebase reserved-claim gotcha:** `user_id` (along with `sub`, `iss`, `aud`, `iat`, `exp`, `email`, `email_verified`, `firebase`, etc.) is a Firebase Auth **reserved** claim — Firebase silently drops a custom `user_id` and substitutes the raw Firebase UID. Memories therefore writes the actor user UUID under the non-reserved name **`actor_user_id`**, which the API reads via `readActorUserIdClaim` (`user_id` is still accepted as a fallback for local-dev tokens issued by `npm run dev:local-auth`). If you bypass the operator script and set claims by hand (e.g. via `firebase auth:import`), you **must** use `actor_user_id`, not `user_id`, or every mutation will return `403 FORBIDDEN`.
...
| Date | Notes |
| --- | --- |
| 2026-05-11 | **Firebase reserved-claim fix (`actor_user_id`):** the Memories API now reads the actor user UUID from `actor_user_id` (preferred) with a fallback to `user_id` for local-dev tokens, because Firebase Auth silently drops a custom `user_id` claim. [`apps/api/scripts/set-memories-firebase-claims.ts`](../apps/api/scripts/set-memories-firebase-claims.ts) writes `actor_user_id`, all four call sites in [`apps/api/src/app.ts`](../apps/api/src/app.ts) use the new `readActorUserIdClaim` helper, and `apps/api/src/app.test.ts` adds two regression tests covering the Firebase token shape. Re-run `npm run set-firebase-claims` for any user provisioned before this change (or re-issue with `actor_user_id`) and have them sign out / back in. Section 3.1 step 5 updated. |
| 2026-05-11 | **Bucket CORS auto-apply:** [`deploy-api.yml`](../.github/workflows/deploy-api.yml) now re-applies [`infra/gcp/cors.json`](../infra/gcp/cors.json) to `gs://${GCP_MEDIA_BUCKET}` after every Cloud Run deploy; [`bootstrap.sh`](../infra/gcp/bootstrap.sh) grants the deploy SA bucket-scoped `roles/storage.admin` so the workflow has `storage.buckets.update`. Section 3.1 step 4 updated. `cors.json` trimmed to real Legacy origins (removed stale template entries). No schema impact; existing buckets receive the new CORS on the next deploy. |
| 2026-05-11 | **CORS wiring:** [`deploy-api.yml`](../.github/workflows/deploy-api.yml) now reads repository variable `WEB_ORIGIN` and sets it on the Cloud Run revision (uses `gcloud --set-env-vars` with a `^|^` delimiter so commas inside the origin list survive). Deploy fails fast if `WEB_ORIGIN` is unset. Inventory and section 3.1 updated; no schema or secret-rotation impact. |
