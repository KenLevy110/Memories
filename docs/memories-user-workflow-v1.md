...
| **Stage alignment** | **Stage 0.5** ships a reduced capability slice (see [development-plan-v1.md](development-plan-v1.md) Section 4). **Stage 0.5.1-demo** (**T25**, tag **`v0.5.1-demo`**) targets **workflow-structure** demos while keeping **0.5** finalize limits; **Stage 0.5.2** (tag **`v0.5.2`**) adds **hi-fi-adjacent** capture styling toward screenshots (not pixel-identical). **Coming Soon** placeholders cover **0.6–1.0** controls until **T15–T21** land. Full functional parity with this doc follows those tickets. |
