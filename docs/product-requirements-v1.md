...
### Dashboard platform integration (v1.1)

Cross-repo **Dashboard → Memories** launch (minted `redirect_url`, **Back to Dashboard**) is specified for the **Memories** side in **[product-requirements-v1.1.md](product-requirements-v1.1.md)** and **[development-plan-v1.1.md](development-plan-v1.1.md)**. The **wire-level** request/response and env keys remain co-owned in the Dashboard repo: **[../Dashboard/docs/memories-integration-contract-v1.1.md](../Dashboard/docs/memories-integration-contract-v1.1.md)** (sibling checkout path).
...
The product is a system to save memories for elders before they part with items where the memories includes pictures, voice recording and/or video recording along with transcripts. This helps the ease of separating from the item.
...
| **NFR-010** | Ops               | Monitor upload/AI failure rates with alerts owned by Memories service dashboards (staging + prod), synchronized with infra on-call rotations; aligns with **[technical-design-v1.md](technical-design-v1.md)** §3.3 + handoff Section 11.3.                                                                                                                                                            | P1  | Starter signal/threshold contract: **[development-plan-v1.md](development-plan-v1.md) §5.1** — tune metric names and burn rates with ops; record owners in **[implementation-log.md](implementation-log.md)** |
| **NFR-011** | Quality / testing | P0 flows (capture, save/idempotency, authz on memory reads/writes) have **automated tests** meeting repo policy (see [AGENTS.md](../AGENTS.md)); critical paths have **manual or E2E** verification before closed-pilot launch—details in **[development-plan-v1.md](development-plan-v1.md)** / TDD, not duplicated here.                                        | P1  | PRDs often state *that* quality is required; *how* stays in plan + stack |
